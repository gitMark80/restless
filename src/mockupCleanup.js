...
/* Keep the homepage clean and remove duplicate promotional blocks. */
...
/* Light mode should resemble the approved mockup: bright paper, navy type, soft blue accents. */
...
body[data-restless-theme="light"] main section.rounded-3xl,
body[data-restless-theme="light"] main .rounded-2xl {
  background-color: #ffffff !important;
}
body[data-restless-theme="light"] main section.rounded-3xl::before {
  background:
    radial-gradient(circle at 82% 22%, rgba(255,226,185,.55), transparent 27%),
    linear-gradient(135deg, rgba(240,248,255,.98), rgba(255,249,239,.96)) !important;
}

/* The question field should read as blue, not yellow/gold. */
body[data-restless-theme="light"] footer textarea {
  background: #eef6ff !important;
  border: 1.5px solid #9ccdf8 !important;
  color: var(--rf-navy) !important;
  box-shadow: inset 0 0 0 1px rgba(32,135,232,.03) !important;
}
...
/* Keep the three homepage promises side-by-side, including on phones. */
...
/* About, Contact and Support use the same white/navy/blue system. */
...
/* Three-dot thinking wave in Restless blue. */
...
@media (max-width: 640px) {
  ...
  main section.rounded-3xl .grid.grid-cols-3 { grid-template-columns: repeat(3,minmax(0,1fr)) !important; }
  main section.rounded-3xl .grid.grid-cols-3 > div { min-height: 64px !important; }
}
`;
...
function cleanLegacyExtras() {
  ...
  if (section) {
    const labels = section.querySelectorAll('.grid.grid-cols-3 span');
    const spanish = Array.from(document.querySelectorAll('button')).some((button) => button.textContent?.trim() === 'EN');
    const copy = spanish ? ['Respuestas', 'Fuentes', 'Próximos pasos'] : ['Answers', 'Sources', 'Next steps'];
    labels.forEach((label, index) => {
      if (copy[index] && label.textContent !== copy[index]) label.textContent = copy[index];
    });
  }
}
